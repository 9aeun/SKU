	...
	<BODY> 
		...
		<TBODY>
			<xsl:apply-templates select = "/제품/핸드폰">
				...
				<xsl:sort order="descending" select="가격" />
			</xsl:apply-templates>
		</TBODY>
		</TABLE>
	</BODY>
...
<xsl:template match = "/제품/핸드폰">
		<TR>
		<TD bgcolor="#DEE3EF"><p align="center" style="margin-top:3px;"><xsl:number format="01" /></p></TD> <!--Number는 original이므로 순서가 오름차순, 내림차순에 맞춰서 나옴 -->
